...
DNSClient::DNSClient(const char* server, const char* hostname) : m_state(MYNETDNS_START), m_udp(NULL) {
    m_hostname = hostname;
    dns_server = server;
}

DNSClient::DNSClient(const char* server, Endpoint* pHost) : m_state(MYNETDNS_START), m_udp(NULL) {
    dns_server = server;
}
...
void DNSClient::resolve(const char* hostname) {
    ...
    Endpoint server;
    server.set_address(dns_server, 53); // DNS
